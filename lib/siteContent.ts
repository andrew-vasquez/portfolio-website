export const heroContent = {
  pill: "Andrew Vasquez | Full-Stack Developer",
  lines: ["I build clean, fast", "web products."],
  description:
    "Frontend-focused full-stack developer building with React, Next.js, TypeScript, and modern web tooling.",
  primaryCta: {
    label: "View Featured Work",
    href: "#featured-work",
  },
  secondaryCta: {
    label: "Email Me",
    href: "mailto:avasdev98@gmail.com",
  },
  meta: [
    "WGU Software Developer student",
    "React, Next.js, TypeScript",
    "Open to frontend and full-stack roles",
  ],
};

export const stackGroups = [
  {
    title: "Frontend",
    items: [
      { name: "HTML", image: "/icons/html.svg" },
      { name: "CSS", image: "/icons/css.svg" },
      { name: "Next.js", image: "/icons/nextjs.svg" },
      { name: "React", image: "/icons/react.svg" },
      { name: "TypeScript", image: "/icons/typescript.svg" },
      { name: "Tailwind", image: "/icons/tailwind.svg" },
      { name: "JavaScript", image: "/icons/javascript.svg" },
    ],
  },
  {
    title: "Backend",
    items: [
      { name: "Node.js", image: "/icons/nodejs.svg" },
      { name: "Hono", image: "/icons/hono.svg" },
      { name: "Prisma", image: "/icons/prisma.svg" },
      { name: "PostgreSQL", image: "/icons/postgresql.svg" },
      { name: "Python", image: "/icons/python.svg" },
    ],
  },
  {
    title: "Workflow",
    items: [
      { name: "Git", image: "/icons/git.svg" },
      { name: "GitHub", image: "/icons/github.svg" },
      { name: "VSCode", image: "/icons/vscode.svg" },
    ],
  },
];

export const contactContent = {
  label: "Contact",
  title: "Get in touch",
  description: "Interested in frontend, full-stack, and product-focused roles.",
};
