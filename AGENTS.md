...
This project uses pnpm (`pnpm-lock.yaml` is committed).
...
If pnpm is unavailable, equivalent npm scripts exist (`npm run dev`, `npm run build`, etc.).
...
- Confirm `pnpm lint` and `pnpm build` pass locally.
